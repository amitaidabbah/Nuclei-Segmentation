...
path_to_data_dir = "drive/My Drive/Colab Notebooks/RSIP/class3_data"
image_name = "img.png"
...
##############################################################################################################
################ DEFINE SEQUNECER ############################################################################
def create_aug_seq(img_seq, mask_seq):
    seq = iaa.Sequential([
    ...
    segmap = ia.SegmentationMapOnImage(segmap, shape=(image_height_for_training, image_width_for_training),
                                       nb_classes=1 + 5)
...
def generate_training_batches(relevant_file_names, batch_size):
    ...
    while True:
        ...
        for batch_index in range(0, num_files - batch_size + 1, batch_size):
            ...
            #             batch_img = seq.augment_images(batch_img)
            #             batch_mask = seq.augment_images(batch_mask)
...
def convolution_block(input_layer, num_filters, kernel_size, activation_func):
    drop1 = Dropout(rate=Rate)(input_layer)
    ...
    drop2 = Dropout(rate=Rate)(conv1)
    conv2 = Conv2D(filters=num_filters, kernel_size=kernel_size, activation=activation_func,
                   kernel_initializer='he_uniform', padding='same')(drop2)
    drop1 = Dropout(rate=Rate)(conv2)
...
def create_and_train_model(training_data, validation_data, input_height, input_width, first_layer_num_filters,
    ...
    model.save('drive/My Drive/Colab Notebooks/RSIP/cell_unet_weights.h5')
